...
function InteractiveRow<T extends Record<string, string>>(
    {
        ...
        return (
            <tr 
                ...
                className={` ${(enableSelection && selectedRow == row.row_key) ? "bg-primary-400/30" : "hover:bg-primary-400/10"} ${enableSelection ? "cursor-pointer" : ""}`}>
                <>
                    {enableSelection && (
                    ...
                    {cols.map(rowValue => (
                        <td className={` border border-gray-400/40 text-left `}>
                            <p className={`p-3 ${(enableSelection && selectedRow == row.row_key) ? " text-gray-500 " : " " } `}>{rowValue}</p>
                        </td>
                    ))}
                </>
            </tr>
            ...
            {!disableHeader && (
                <thead>
                    <tr className=" bg-dark-200">
                        {enableSelection && (<th className="border border-gray-400/40 text-left"></th>)}
                        {columns.map(column => (
